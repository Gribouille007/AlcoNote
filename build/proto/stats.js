...
function _flatEntries(families) {
  const out = [];
  for (const f of families) for (const e of f.entries || []) out.push({
    ...e,
    family: f
  });
  return out;
...
function _entryVolumeCl(e) {
  const f = e.family;
  return f.unit === 'EcoCup' ? f.quantity * 25 : f.unit === 'L' ? f.quantity * 100 : f.quantity;
}
...
function deriveSessionDurationBuckets(families) {
  ...
  for (const sess of _sessions(_flatEntries(families))) {
    ...
    const idx = span < 1 ? 0 : span < 2 ? 1 : span < 3 ? 2 : span < 4 ? 3 : span < 5 ? 4 : span < 6 ? 5 : 6;
    buckets[idx].v += 1;
  }
  return buckets;
}
function deriveSessionBacBuckets(families) {
  ...
  for (const sess of _sessions(_flatEntries(families))) {
    ...
    const idx = peakGperL < 0.2 ? 0 : peakGperL < 0.5 ? 1 : peakGperL < 0.8 ? 2 : peakGperL < 1.2 ? 3 : 4;
    buckets[idx].v += 1;
  }
  return buckets;
}
function deriveBacProjection(families) {
  ...
  const horizonH = 12;
  const now = Date.now();
  const entries = _flatEntries(families).filter(e => {
    const ts = Date.parse(e.ts);
...
function PeriodNav({
  ...
  const label = period === 'week' ? '13 – 19 avril 2026' : period === 'day' ? "Samedi 18 avril" : period === 'month' ? 'Avril 2026' : period === 'year' ? '2026' : 'Depuis le début';
  const arrowBtn = icon => /*#__PURE__*/React.createElement("div", {
    style: {
      width: 32,
      height: 32,
      borderRadius: 10,
  ...
  }, arrowBtn(Ic.chevL), /*#__PURE__*/React.createElement("div", {
    style: {
...
function GeneralSection({
  ...
  const cards = [{
    v: totalDrinks,
    l: 'Boissons',
    tip: 'Total consommations'
  }, {
    v: sessions.length,
    l: 'Sessions',
    tip: 'Regroupées si < 4h'
  }, {
    v: fmtL(totalVolumeCl),
    l: 'Volume'
  }, {
    v: `${Math.round(totalGrams)}g`,
    l: 'Alcool pur',
    tip: 'Éthanol'
  }, {
    v: uniqueDrinks,
  ...
  const catDist = [...catDistMap.entries()].map(([name, v]) => ({
    name,
    v
  })).sort((a, b) => b.v - a.v);
  ...
  }, c.l), !hidePct && c.comp !== undefined && c.comp !== 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      position: 'absolute',
      top: 10,
      right: 10,
      fontSize: 9,
      color: c.comp > 0 ? T.accent2 : T.good,
      fontFamily: fontNum,
      fontWeight: 500
    }
  }, c.comp > 0 ? '↑' : '↓', " ", Math.abs(c.comp), "%")))), /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("div", {
    style: {
      color: T.ink,
      fontSize: 13,
      fontWeight: 500,
...
function TemporalSection({
  ...
  const avgSessionLabel = avgSessionH > 0 ? `${Math.floor(avgSessionH)}h ${String(Math.round(avgSessionH % 1 * 60)).padStart(2, '0')}` : '—';
  ...
  return /*#__PURE__*/React.createElement(StatSection, {
    ...
    big: hourly.some(v => v) ? `${peakHour}h` : '—',
    label: "Heure de pointe"
  }), /*#__PURE__*/React.createElement(MiniStat, {
    big: peakDay.v ? dayNames[peakDay.day] : '—',
...
function CategorySection({
  ...
  const cats = [...byCat.entries()].map(([name, a]) => {
    ...
    return {
      name,
      count: a.count,
      volume: a.volumeCl / 100,
      // L
      avgVol: a.volumeCl / a.count / 100,
      // L
  ...
  return /*#__PURE__*/React.createElement(StatSection, {
    ...
    label: "Volume",
    value: `${c.volume.toFixed(1)}L`
  }), /*#__PURE__*/React.createElement(StatRow, {
    label: "Volume moyen",
    value: `${c.avgVol.toFixed(2)}L`
  }), /*#__PURE__*/React.createElement(StatRow, {
...
function TopDrinksSection({
  ...
  }, d.count, " fois \xB7 ", d.volume.toFixed(2), "L \xB7 ", fmtDate(d.last))), /*#__PURE__*/React.createElement("div", {
    style: {
      fontFamily: fontNum,
      color: T.accent,
      fontSize: 13,
...
function BACSection({
  ...
  const relevantDrinks = _flatEntries(families).filter(e => {
    const ts = Date.parse(e.ts);
    return !Number.isNaN(ts) && now - ts >= 0 && (now - ts) / 3600000 < 12;
  }).sort((a, b) => Date.parse(b.ts) - Date.parse(a.ts)).slice(0, 6).map(e => ({
    name: e.family.name,
    qty: fmtQty(e.family),
  ...
  return /*#__PURE__*/React.createElement(StatSection, {
  ...
  }, fmtTime(hoursToLegal))))), /*#__PURE__*/React.createElement(Card, {
    style: {
      marginBottom: 10
    }
  }, /*#__PURE__*/React.createElement("div", {
    ...
    points: deriveBacProjection(families),
    width: 320,
    height: 130,
    now: 0
  }), /*#__PURE__*/React.createElement("div", {
  ...
  }, "Glissez pour voir le taux \xE0 un moment pr\xE9cis")), /*#__PURE__*/React.createElement(Card, {
    style: {
      marginBottom: 10
    }
  }, /*#__PURE__*/React.createElement("div", {
...
function BACRecordRow({
  ...
  const months = ['janv.', 'févr.', 'mars', 'avr.', 'mai', 'juin', 'juil.', 'août', 'sept.', 'oct.', 'nov.', 'déc.'];
  const dateLabel = daysAgo === 0 ? `Aujourd'hui à ${d.getHours()}h${String(d.getMinutes()).padStart(2, '0')}` : daysAgo === 1 ? `Hier à ${d.getHours()}h${String(d.getMinutes()).padStart(2, '0')}` : daysAgo < 7 ? `${daysAgo} jours · ${d.getHours()}h${String(d.getMinutes()).padStart(2, '0')}` : `${d.getDate()} ${months[d.getMonth()]} · ${d.getHours()}h${String(d.getMinutes()).padStart(2, '0')}`;
  return /*#__PURE__*/React.createElement("div", {
    style: {
      position: 'relative',
      overflow: 'hidden',
  ...
  }, record.bac, /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 10,
      color: T.muted,
      fontStyle: 'normal',
  ...
  }, dateLabel, " \xB7 ", record.drinks, " conso", record.drinks > 1 ? 's' : ''))));
}
...
function AdvancedSection({
  ...
  }, "Alcool quotidien liss\xE9 sur 7 et 30 jours"), /*#__PURE__*/React.createElement(RollingChart, {
    families: families
  }), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
  ...
  }, "Dur\xE9e et alcool\xE9mie moyenne par session"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'grid',
      gridTemplateColumns: '1fr 1fr',
      gap: 10
...
function RollingChart({
  ...
  return /*#__PURE__*/React.createElement("svg", {
    ...
    strokeDasharray: "3 2",
    strokeLinejoin: "round"
  }), n > 0 && [0, Math.floor(n / 2), n - 1].map((i, k) => /*#__PURE__*/React.createElement("text", {
    key: k,
    x: xs(i),
    y: height - 8,
    fontSize: 9,
