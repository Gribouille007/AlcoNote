...
function computeBACSessions(drinks, weight = DEFAULT_WEIGHT_KG, gender = 'male') {
  ...
  const bps = [];
  for (const d of valid) {
    bps.push({ t: d._ts, dr: d._peak / T, d });          // absorption begins
    bps.push({ t: d._ts + T * hour, dr: -d._peak / T });  // absorption ends
  }
  ...
  for (const bp of bps) {
    ...
    if (bp.d) {
      if (!cur) {
        cur = {
          id: 'sess::' + bp.d._ts,
          startTs: bp.d._ts,
          endTs: bp.d._ts,        // provisional, set when the session closes
          lastDrinkTs: bp.d._ts,
          drinks: [bp.d],
          grams: bp.d._grams,
          peakBac: bac,
          peakTs: bp.d._ts,
          area: 0,                // ∫ bac dt sur la session (mg/L·h)
    ...
    if (cur && bac > cur.peakBac) { cur.peakBac = bac; cur.peakTs = bp.t; }
  }
...
function StatsTab({ storageScope = '', hideMap = false, hideBac = false, hidePrice = false, bacAvailable = true, reorderRef } = {}) {
  ...
  const visibleSections = React.useMemo(() => {
    const flags = { hideBac, hideMap, hidePrice };
    return sectionOrder
      .map(id => STATS_SECTIONS.find(s => s.id === id))
      .filter(s => s && !(s.hide && s.hide(flags)));
...
function useSectionOrder() {
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      ...
      if (!db || !alive) return;
      let saved = null;
      try { saved = JSON.parse((await db.getSetting(SECTION_ORDER_KEY)) || 'null'); } catch {}
      if (alive) setOrder(normalizeSectionOrder(saved));
    };
    load();
    const unsub = dataBus.sub((ch) => { if (ch === 'settings' || ch == null) load(); });
...
function SectionReorderList({ sections, onChange }) {
  ...
  return (
    <div style={{ position: 'relative', height: sections.length * REORDER_STEP - REORDER_GAP }}>
      {sections.map((s, i) => {
        const isDragged = drag != null && drag.id === s.id;
        const y = isDragged
          ? drag.from * REORDER_STEP + drag.dy
          : displayIds.indexOf(s.id) * REORDER_STEP;
        return (
          <div key={s.id} style={{
...
function computeBacForecast(currentBac, allSessions, weight, gender, nowMs, futurePoints = []) {
  ...
  const meanPeakBac = hasHistory
    ? past.reduce((s, x) => s + (x.peakBac || 0), 0) / past.length
    : null;
  ...
  const meanDrinkingMs = hasHistory
    ? past.reduce((s, x) => s + Math.max(0, (x.lastDrinkTs || x.startTs) - x.startTs), 0) / past.length
    : null;
  ...
  const estStopMs = meanDrinkingMs != null
...
function saveForecastEnabled(enabled) {
  try { localStorage.setItem(FORECAST_ENABLED_KEY, enabled ? '1' : '0'); } catch {}
}
...
function ForecastToggle({ enabled, onChange }) {
...
function ForecastMiniStats({ forecast, fmtTime }) {
  const rate = forecast.hasCurrentSession
    ? `${forecast.currentRateGph.toFixed(1)} g/h`
    : '—';
  const duration = forecast.meanDrinkingMs != null
    ? fmtTime(forecast.meanDrinkingMs / 3600_000)
    : '—';
  const peak = forecast.meanPeakBac != null
    ? `${Math.round(forecast.meanPeakBac)}`
    : '—';
  const eta = forecast.etaPeakHours === 0
    ? 'maintenant'
    : forecast.etaPeakHours != null
    ? fmtTime(forecast.etaPeakHours)
    : (forecast.meanPeakBac != null && forecast.hasCurrentSession ? '∞' : '—');
  ...
  return (
    <div style={{
      ...
      {item(Ic.clock,     'ETA peak',  eta,      null)}
    </div>
  );
}

function BACSection({ collapsed, toggleSection, allSessions, sessions, prevSessions, period, weight, gender }) {
  ...
  const sortedRecords = React.useMemo(() => {
    return (allSessions || [])
      .filter(s => s.peakBac >= BAC_RECORD_MIN)
      .map(s => ({
        id: s.id,
  ...
  const totalRecords = React.useMemo(
    () => (allSessions || []).filter(s => s.peakBac >= BAC_RECORD_MIN).length,
    [allSessions]
  );
    ...
    <StatSection id="bac" title="Alcoolémie" collapsed={collapsed} toggleSection={toggleSection} sub="Estimation BAC · Formule de Widmark">
      ...
      {forecast.hasCurrentSession && (
        <Card style={{ marginBottom: 10 }}>
          ...
          {forecastEnabled && (
            <>
              <ForecastMiniStats forecast={forecast} fmtTime={fmtTime} />
      ...
      {sortedRecords.length > 0 && (
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            {others.map(r => <BACRecordRow key={r.id} record={r} />)}
          </div>
        </div>
      )}
    </StatSection>
...
function MapSection({ drinks, allDrinks, collapsed, toggleSection }) {
  ...
  React.useEffect(() => {
    ...
    const cluster = L.markerClusterGroup({
      ...
      spiderfyOnMaxZoom: false,    // taper un rond ouvre la liste, pas d'éclatement
      zoomToBoundsOnClick: false,  // … et ne zoome pas non plus dessus
      maxClusterRadius: 44,
      iconCreateFunction: (c) => {
        ...
        return L.divIcon({
          html: `<svg viewBox="0 0 ${size} ${size}" width="${size}" height="${size}">`
            ...
            + `<text x="${cx}" y="${cx}" text-anchor="middle" dominant-baseline="central" font-size="${fs}">${n}</text>`
            + `</svg>`,
    ...
    return () => {
      ...
      tileRef.current = null; boundsRef.current = null;
    };
  }, [isOpen, ready]);
  ...
  React.useEffect(() => {
    ...
    if (mode === 'heat' && typeof L.heatLayer === 'function') {
      ...
      for (const p of geoPoints) {
        ...
        const mk = L.marker([p.lat, p.lng], { icon: dotIcon, drink: p.d });
  ...
  return (
    ...
    {listDrinks && (
      <MapDrinksSheet drinks={listDrinks} onClose={() => setListDrinks(null)} />
    )}
    </>
  );
}
...
function bucketSpend(priced, period, range) {
  ...
  if (period === 'today') {
    ...
    const data = [];
    for (let h = 0; h < 24; h++) data.push({ label: `${h}h`, fullLabel: `${h}h`, v: round2(sum[String(h)] || 0) });
    return { data, unitLabel: 'par heure', formatX: (d, i) => (i % 3 === 0 ? d.label : '') };
  }

  const granularity = (period === 'year' || period === 'school') ? 'month'
    : period === 'all' ? 'year' : 'day';

  if (granularity === 'day') {
    for (const d of priced) { if (d.date) add(d.date, Number(d.price)); }
    ...
    for (let dt = new Date(start); dt <= last; dt = _addDays(dt, 1)) {
      const k = _fmtIso(dt);
      data.push({ label: `${dt.getDate()}/${dt.getMonth() + 1}`, fullLabel: `${dt.getDate()} ${FR_MONTHS_SHORT[dt.getMonth()]}`, v: round2(sum[k] || 0) });
    }
    return { data, unitLabel: 'par jour', formatX: null };
  }

  if (granularity === 'month') {
    for (const d of priced) { if (d.date) add(d.date.slice(0, 7), Number(d.price)); }
    ...
    for (let dt = new Date(start); dt <= end; dt.setMonth(dt.getMonth() + 1)) {
      ...
      data.push({ label: multiYear ? `${base} ${String(y).slice(2)}` : base, fullLabel: `${base} ${y}`, v: round2(sum[k] || 0) });
    }
    return { data, unitLabel: 'par mois', formatX: (d, i) => (i % 2 === 0 ? d.label : '') };
  }

  // année (Tout)
  for (const d of priced) { if (d.date) add(d.date.slice(0, 4), Number(d.price)); }
  ...
  for (let y = years.length ? years[0] : 0; years.length && y <= years[years.length - 1]; y++) {
    data.push({ label: `'${String(y).slice(2)}`, fullLabel: String(y), v: round2(sum[String(y)] || 0) });
  }
  return { data, unitLabel: 'par année', formatX: null };
}
