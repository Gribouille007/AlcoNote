...
function computeBacForecast(currentDrinks, currentBac, allSessions, weight, gender, nowMs) {
  ...
  const valid = (currentDrinks || [])
    .filter(d => d.date && d.time)
    .map(d => ({ ...d, _ts: new Date(`${d.date}T${d.time}`).getTime() }))
    .filter(d => Number.isFinite(d._ts) && d._ts <= nowMs)
    .sort((a, b) => a._ts - b._ts);
  ...
  const meanPeakBac = hasHistory
    ...
    : null;
  const meanDurationMs = hasHistory
    ? past.reduce((s, x) => s + Math.max(0, x.endTs - x.startTs), 0) / past.length
    : null;

  if (!hasCurrentSession) {
    return {
      currentRateGph: 0,
      meanDurationMs, estStopMs: null, meanPeakBac,
  ...
  const totalGrams = valid.reduce(
    (s, d) => s + toCl(d.quantity, d.unit) * 10 * ((d.alcoholContent || 0) / 100) * 0.789,
    0
  );
  ...
  const estStopMs = meanDurationMs != null
    ? Math.max(nowMs, sessionStartMs + meanDurationMs)
    : nowMs;
  ...
  return {
    currentRateGph, meanDurationMs, estStopMs, meanPeakBac,
...
function ForecastMiniStats({ forecast, fmtTime }) {
  const rate = forecast.hasCurrentSession
    ...
    : '—';
  const duration = forecast.meanDurationMs != null
    ? fmtTime(forecast.meanDurationMs / 3600_000)
    : '—';
  const peak = forecast.meanPeakBac != null
    ? `${Math.round(forecast.meanPeakBac)}`
    : '—';
  const eta = forecast.etaPeakHours != null
...
function BACSection({ collapsed, toggleSection, allSessions, weight, gender }) {
  ...
  const forecast = React.useMemo(
    () => computeBacForecast(bacInfo.drinks, currentBAC, allSessions, weight, gender, Date.now()),
    [bacInfo.drinks, currentBAC, allSessions, weight, gender]
  );
  const realPastPoints = React.useMemo(
    () => (bacInfo.points || []).filter(p => p.t <= 0),
    [bacInfo.points]
  );
