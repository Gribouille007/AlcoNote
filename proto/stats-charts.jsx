...
function SvgBACProjection({ points, width = 320, height = 120, now = 0.3 }) {
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} width="100%" height={height} style={{ display: 'block' }}>
      ...
      <circle cx={xs(now)} cy={ys(points.find(p => Math.abs(p.t - now) < 0.01)?.bac || points[0].bac)}
        r={3.5} fill={T.accent} />
...
Object.assign(window, {
  niceMax, SvgBarChart, SvgRadar, SvgDonut, SvgLineChart,
