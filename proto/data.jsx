...
function suggestPriceForVolume(families, name, volCl, alcohol) {
  ...
  return {
    ...
    exact,
    source: { name: src.f.name, quantity: src.f.quantity, unit: src.f.unit, referencePrice: src.ref },
  };
}
...
async function setCategoryColor(id, hue) {
  ...
  if (!db) throw new Error('Base de données indisponible');
  if (id == null || id === '') throw new Error('Catégorie invalide');
  const val = (hue == null || !Number.isFinite(Number(hue)))
    ? null
    : String(((Math.round(Number(hue)) % 360) + 360) % 360);
...
function CategoryIconsProvider({ children }) {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (alive) { setIdMap(icons); setColorMap(colors); }
      } catch {
        if (alive) { setIdMap({}); setColorMap({}); }
      }
    })();
    return () => { alive = false; };
  }, [v]);
  ...
  React.useMemo(() => { applyCatHueOverrides(colorByName); return null; }, [colorByName]);
  return React.createElement(
    CategoryColorsContext.Provider, { value: colorByName },
    React.createElement(CategoryIconsContext.Provider, { value: map }, children)
  );
}
...
Object.assign(window, {
  ...
  applyReferenceToFamily, suggestPriceForVolume,
  ratingKey,
  saveSetting,
