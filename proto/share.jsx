...
const SHARE_CFG = Object.assign(
  { TRANSPORT: 'mock', SUPABASE_URL: '', SUPABASE_ANON_KEY: '', PULL_INTERVAL_MS: 10 * 60 * 1000 },
...
async function _loadPubIndex() {
  try { return JSON.parse((await _sget('share.pubindex')) || '{}') || {}; } catch { return {}; }
}
async function _savePubIndex(idx) { await _sset('share.pubindex', JSON.stringify(idx)); }
...
async function _ratingsMap(db) {
  ...
  try {
    ...
    for (const r of all) {
      ...
      if (out[k] == null || t >= (out[k]._t || 0)) out[k] = { v: r.rating, _t: t };
    }
  } catch (e) {}
  const flat = {};
...
function MockShareTransport() {
  ...
  function seed(groupId) {
    const now = Date.now();
    const min = 60000, hr = 3600000, day = 24 * hr;
    let n = 0;
    const mk = (author, name, qty, unit, cl, abv, cat, rating, ms) => {
      const d = new Date(ms);
      return {
        uid: `mock-${author}-${ms}-${(n++).toString(36)}`,
        groupId, authorId: author, tsUtc: ms,
        date: localDate(d), time: localTime(d),
        name, quantity: qty, unit, quantityInCL: cl,
        alcoholContent: abv, category: cat, rating, updatedAt: ms, deleted: false,
      };
    };
    const lea = [
      mk('mock-lea', 'La Chouffe', 33, 'cL', 33, 8, 'Bière', 5, now - 35 * min),
      mk('mock-lea', 'La Chouffe', 33, 'cL', 33, 8, 'Bière', 5, now - 75 * min),
      mk('mock-lea', 'Spritz', 1, 'EcoCup', 25, 11, 'Cocktail', 4, now - 2 * hr - 20 * min),
      mk('mock-lea', 'Côtes du Rhône', 12, 'cL', 12, 13, 'Vin', 4, now - 2 * day),
      mk('mock-lea', 'Mojito', 1, 'EcoCup', 25, 10, 'Cocktail', 3, now - 5 * day),
      mk('mock-lea', 'La Chouffe', 33, 'cL', 33, 8, 'Bière', 5, now - 6 * day - 2 * hr),
    ];
    const tom = [
      mk('mock-tom', 'IPA Brewdog', 50, 'cL', 50, 6.5, 'Bière', 4, now - 18 * min),
      mk('mock-tom', 'Whisky', 4, 'cL', 4, 40, 'Spiritueux', 5, now - 3 * hr),
      mk('mock-tom', 'IPA Brewdog', 50, 'cL', 50, 6.5, 'Bière', 4, now - 1 * day - 1 * hr),
      mk('mock-tom', 'Heineken', 25, 'cL', 25, 5, 'Bière', 2, now - 4 * day),
      mk('mock-tom', 'Rhum', 4, 'cL', 4, 40, 'Spiritueux', 3, now - 8 * day),
    ];
    return {
      groupId,
      members: [
        { userId: 'mock-lea', displayName: 'Léa', shareBac: true, bacWeight: 62, bacGender: 'female' },
        { userId: 'mock-tom', displayName: 'Tom', shareBac: false, bacWeight: null, bacGender: null },
      ],
      drinks: [...lea, ...tom],
      mine: [],
  ...
  return {
    ...
    async createGroup() {
      ...
      write(srv);
      return { groupId, inviteCode: _inviteCode() };
    },
    async joinGroup(/* code */) {
      // En mock on (re)seed le même groupe de démonstration (créateur inconnu).
      let srv = read();
      if (!srv) { srv = seed('grp-demo'); write(srv); }
      srv.kickedSelf = false; // revenir dans le groupe ré-active ma membership
      write(srv);
      return { groupId: srv.groupId, inviteCode: _inviteCode() };
    },
    async leaveGroup() { try { localStorage.removeItem(KEY); } catch {} },
    ...
    async pushUpserts(records) {
      ...
      srv.mine = [...byUid.values()]; write(srv);
    },
    async pushTombstones(uids) {
      const srv = read(); if (!srv) return;
      const set = new Set(uids);
      srv.mine = srv.mine.map(r => set.has(r.uid) ? { ...r, deleted: true, updatedAt: Date.now() } : r);
      write(srv);
    },
    async pullSince(cursor, { withMeta = true } = {}) {
      const srv = read();
      if (!srv) {
        return {
          drinks: [], profiles: [], members: withMeta ? [] : undefined,
          creatorId: withMeta ? null : undefined, authUserId: shareState.userId,
          cursor: cursor || 0, error: null,
        };
      }
      ...
      const selfRow = srv.kickedSelf ? [] : [{
        userId: shareState.userId, displayName: shareState.displayName || '',
        shareBac: !!shareState.shareBac, bacWeight: null, bacGender: null,
      }];
    ...
    async importRecovery(blob) { if (blob && blob.uid) localStorage.setItem('alconote.mock.uid', blob.uid); },
  };
}
...
function ensureSupabaseSdk() {
  ...
  _sdkPromise = new Promise((resolve, reject) => {
    ...
    s.async = true;
    s.onload = () => resolve();
    s.onerror = () => { _sdkPromise = null; reject(new Error('Chargement SDK Supabase échoué')); };
...
function SupabaseShareTransport(cfg) {
  ...
  return {
    ...
    async ensureIdentity() {
      ...
      return { userId: (data.session || data).user.id };
    },
    async setProfile(p) {
      const sb = await ensureClient();
      const { error } = await sb.from('shared_profiles').upsert({
        user_id: p.userId, group_id: p.groupId, display_name: p.displayName,
        share_enabled: p.shareEnabled, share_bac: p.shareBac,
    ...
    async joinGroup(code) {
      const sb = await ensureClient();
      const { data, error } = await sb.rpc('join_group', { invite_token: code });
      if (error) throw error;
      return { groupId: data.group_id, inviteCode: code };
    },
    async leaveGroup() {
      const sb = await ensureClient();
      const { error } = await sb.rpc('leave_group', { p_group_id: shareState.groupId });
      if (error) throw error;
    },
    ...
    async removeMember(groupId, userId) {
      const sb = await ensureClient();
      const { error } = await sb.rpc('remove_member', { p_group_id: groupId, p_user_id: userId });
      if (error) throw error;
    },
    async pushUpserts(records) {
      const sb = await ensureClient();
      const rows = records.map(r => ({
        uid: r.uid, group_id: r.groupId, author_id: r.authorId, ts_utc: r.tsUtc,
        date: r.date, time: r.time, name: r.name, quantity: r.quantity, unit: r.unit,
        quantity_in_cl: r.quantityInCL, alcohol_content: r.alcoholContent,
        category: r.category, rating: r.rating, updated_at: r.updatedAt, deleted: false,
      }));
      const { error } = await sb.from('shared_drinks').upsert(rows, { onConflict: 'uid' });
      if (error) throw error;
    },
    async pushTombstones(uids) {
      const sb = await ensureClient();
      const { error } = await sb.from('shared_drinks')
        .update({ deleted: true, updated_at: Date.now() }).in('uid', uids);
      if (error) throw error;
    },
    async pullSince(cursor, { withMeta = true } = {}) {
      const sb = await ensureClient();
      ...
      const metaQueries = withMeta ? [
        sb.from('shared_profiles').select('user_id, display_name, share_bac, bac_weight, bac_gender').eq('group_id', gid),
        sb.from('group_members').select('user_id, display_name').eq('group_id', gid),
        sb.from('groups').select('created_by').eq('id', gid).maybeSingle(),
      ] : [];
      const [d, p, m, g] = await Promise.all([
        sb.from('shared_drinks').select('*').eq('group_id', gid).gt('updated_at', cursor || 0).order('updated_at', { ascending: true }).limit(1000),
        ...metaQueries,
      ]);
      ...
      const drinks = (d.data || []).map(r => ({
        ...
        date: r.date, time: r.time, name: r.name, quantity: r.quantity, unit: r.unit,
        quantityInCL: r.quantity_in_cl, alcoholContent: r.alcohol_content,
        category: r.category, rating: r.rating, updatedAt: r.updated_at, deleted: !!r.deleted,
      }));
      const profById = {};
      ...
      const members = (m && !m.error && Array.isArray(m.data))
        ? m.data.map(mm => {
            ...
            return {
              ...
              userId: mm.user_id, displayName: pr.display_name || mm.display_name || '',
              shareBac: !!pr.share_bac, bacWeight: pr.bac_weight, bacGender: pr.bac_gender,
            };
          })
        : null;
    ...
    async exportRecovery() {
      ...
      return { kind: 'supabase', session: data && data.session ? data.session : null };
    },
...
function getTransport() {
  ...
  if (SHARE_CFG.TRANSPORT === 'supabase' && SHARE_CFG.SUPABASE_URL) transport = SupabaseShareTransport(SHARE_CFG);
  else transport = MockShareTransport();
  return transport;
}
...
//  Moteur de sync
// ════════════════════════════════════════════════════════════════════════
let _recTimer = null, _flushTimer = null, _pullRetry = null, _timer = null;
let _flushing = false, _pulling = false;

function scheduleReconcile() { clearTimeout(_recTimer); _recTimer = setTimeout(reconcile, 600); }
  ...
  try {
    ...
    for (const d of drinks) {
      ...
      if (!prev || prev.u !== u || prev.r !== rating) {
        ...
        idx[d.uid] = { u, r: rating };
      }
    }
    const tombs = [];
    for (const uid of Object.keys(idx)) {
      if (!seen.has(uid)) { tombs.push(uid); delete idx[uid]; }
    }
...
async function flushOutbox(attempt) {
  ...
  try {
    ...
    for (const it of items) {
      if (it.op === 'upsert' && it.records && it.records.length) await getTransport().pushUpserts(it.records);
      else if (it.op === 'tomb' && it.uids && it.uids.length) await getTransport().pushTombstones(it.uids);
      done.push(it.id);
    }
...
async function pull(attempt) {
  ...
  if (typeof navigator !== 'undefined' && navigator.onLine === false) {
    if (shareState.online !== false) { shareState.online = false; _emit(); }
    return;
  }
  _pulling = true; shareState.syncing = true; _emit();
  try {
    await flushOutbox();
    const db = await waitForDb();
    ...
    if (partialErr) {
      ...
      try { console.error('[AlcoNote partage] lecture partielle', partialErr); } catch (_) {}
...
async function ensureIdentityNow() {
  ...
  const { userId } = await getTransport().ensureIdentity();
...
async function publishMyProfile() {
  ...
  try {
    ...
    try { console.warn('[AlcoNote partage] publication du profil échouée', e); } catch (_) {}
  }
}
...
const shareEngine = {
  ...
  async joinGroup(code) {
    await ensureIdentityNow();
    const { groupId, inviteCode } = await getTransport().joinGroup((code || '').trim().toUpperCase());
    shareState.groupId = groupId;
  ...
  async exportRecovery() {
    ...
    return { v: 1, userId: shareState.userId, displayName: shareState.displayName, groupId: shareState.groupId, transport: t };
  },
  async importRecovery(blob) {
    if (!blob) return;
    if (blob.transport) { try { await getTransport().importRecovery(blob.transport); } catch (e) {} }
    if (blob.userId) { shareState.userId = blob.userId; await _sset('share.userId', blob.userId); }
    if (blob.displayName) { shareState.displayName = blob.displayName; await _sset('share.displayName', blob.displayName); }
    if (blob.groupId) { shareState.groupId = blob.groupId; await _sset('share.groupId', blob.groupId); }
    _emit();
    if (shareState.enabled && shareState.groupId) { startTimer(); pull(); }
  },
};
...
async function initShare() {
  try {
    ...
    if (!shareState.userId && shareState.enabled) {
      try { await ensureIdentityNow(); } catch (e) { /* réessayé à l'activation */ }
    }
...
function useGroupMembers() {
  ...
  return React.useMemo(
    () => (s.members || []).filter(m => m.userId !== s.userId),
    [s.members, s.userId]
  );
}
...
function useFavoriteFriend() {
  ...
  return React.useMemo(
    () => ((s.enabled && s.groupId && s.favoriteId)
      ? (members.find(m => m.userId === s.favoriteId && m.shareBac) || null)
      : null),
    [s.enabled, s.groupId, s.favoriteId, members]
  );
}
...
function useSharedDrinks(authorId) {
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      ...
      const mine = all
        .filter(r => r.authorId === authorId && !r.deleted)
        .map(r => ({ ...r, id: r.uid }));
...
function useFriendsBac(members) {
  ...
  const key = (members || [])
    .map(m => `${m.userId}:${m.shareBac ? 1 : 0}:${m.bacWeight || ''}:${m.bacGender || ''}`)
    .join(',');
  React.useEffect(() => {
    ...
    if (!members || members.length === 0) { setMap({}); return; }
    let alive = true;
    const compute = async () => {
      const db = await waitForDb();
      if (!db) return;
      ...
      for (const m of (members || [])) {
        if (!m.shareBac) { out[m.userId] = null; continue; }
...
Object.assign(window, {
  ...
  useSharedDrinks, useSharedRatings, useFriendsBac,
