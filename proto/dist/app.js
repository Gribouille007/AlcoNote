...
function AppShell() {
  ...
  React.useEffect(() => {
    ...
    const t = setTimeout(() => setEntered(prev => prev.has(tab) ? prev : new Set(prev).add(tab)), MOTION.base);
  ...
  const tabContainer = id => ({
    ...
    animation: reducedMotion || entered.has(id) ? undefined : `alcoRise ${MOTION.base}ms ${MOTION.ease}`
  });
