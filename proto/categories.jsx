...
function EditCategorySheet({ category, onClose, mode = 'edit', onRenamed }) {
  ...
  const setHue = (h) => { userTouchedColorRef.current = true; setHueState(h); };
  ...
  const hueTrack = `linear-gradient(90deg, ${
    Array.from({ length: 13 }, (_, i) => `oklch(${pL}% ${pC} ${i * 30}) ${(i / 12 * 100).toFixed(1)}%`).join(', ')
  })`;
  ...
  const onTrackKey = (e) => {
    let h = effHue;
    if (e.key === 'ArrowLeft' || e.key === 'ArrowDown') h = effHue - 1;
    else if (e.key === 'ArrowRight' || e.key === 'ArrowUp') h = effHue + 1;
    else if (e.key === 'Home') h = 0;
    else if (e.key === 'End') h = 359;
    else if (e.key === 'PageUp') h = effHue + 15;
    else if (e.key === 'PageDown') h = effHue - 15;
    else return;
    e.preventDefault();
    setHue(clampHue(h));
  };
      ...
      <div style={{
        ...
        <div style={{
          ...
          <button type="button" onClick={() => setHue(null)}
            ...
            style={{
              ...ghostButton, display: 'inline-flex', alignItems: 'center', gap: 5,
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 22 }}>
          {/* Aperçu : pastille recolorée + nom dans la teinte */}
          <div style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div
              ...
              role="slider"
              tabIndex={0}
              aria-label="Teinte de la catégorie"
              aria-valuemin={0}
              aria-valuemax={359}
              ...
              <div aria-hidden="true" style={{
                ...
                background: previewColor, border: `2px solid ${T.bg}`,
